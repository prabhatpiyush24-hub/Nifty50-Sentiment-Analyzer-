...
from sentiment_auditor import run_sentiment_audit
...
with st.sidebar:
    ...
    Headlines are fetched directly from live RSS feeds<br>
    (Google News & Yahoo Finance).<br><br>
...
tab_overview, tab_deepdive, tab_research = st.tabs(["📊 Market Overview", "🔬 Single-Stock Deep Dive", "🔬 Sentiment Research"])
...
# =============================================================================
# TAB 3: SENTIMENT RESEARCH
# =============================================================================

with tab_research:

    st.markdown('<div class="section-header">🔬 Sentiment Edge Cases & NLP Research</div>', unsafe_allow_html=True)
    st.markdown('<div class="help-text">This tab focuses purely on how the FinBERT model interprets live news. It highlights extreme examples of positive and negative sentiment, and identifies "confusing" headlines where the model struggles to classify the text definitively. This is useful for auditing the NLP pipeline.</div>', unsafe_allow_html=True)

    if headline_details:
        all_hl_df = pd.concat(headline_details.values())
        all_hl_df = all_hl_df.rename(columns={
            "label": "sentiment",
            "p_positive": "pos_prob",
            "p_negative": "neg_prob",
            "p_neutral": "neu_prob"
        ...
        audit_results = run_sentiment_audit(all_hl_df)
        
        if audit_results:
            st.markdown(f"**Total Live Headlines Analyzed:** {audit_results['total_analyzed']}")
            
            st.markdown('<br><div class="section-header">🔥 Extreme Confidence</div>', unsafe_allow_html=True)
            st.markdown('<div class="help-text">Headlines where FinBERT was almost certain of the sentiment direction.</div>', unsafe_allow_html=True)
            
            c_pos, c_neg = st.columns(2)
            
            with c_pos:
                st.markdown(f"#### Top 5 Most Positive")
                for item in audit_results["high_confidence_positive"]:
                    st.markdown(f"""
                    <div style="background: rgba(0, 200, 83, 0.1); padding: 10px; border-radius: 8px; margin-bottom: 8px; border-left: 4px solid {COLORS['positive']}">
                        <strong style="color: {COLORS['positive']}">{item['ticker']}</strong>: {item['headline']}<br>
                        <span style="font-size: 0.8em; color: #A0A0B0;">P(Pos): {item['pos_prob']:.4f} | {item['published']}</span>
                    </div>
                    """, unsafe_allow_html=True)
                    
            with c_neg:
                st.markdown(f"#### Top 5 Most Negative")
                for item in audit_results["high_confidence_negative"]:
                    st.markdown(f"""
                    <div style="background: rgba(255, 23, 68, 0.1); padding: 10px; border-radius: 8px; margin-bottom: 8px; border-left: 4px solid {COLORS['negative']}">
                        <strong style="color: {COLORS['negative']}">{item['ticker']}</strong>: {item['headline']}<br>
                        <span style="font-size: 0.8em; color: #A0A0B0;">P(Neg): {item['neg_prob']:.4f} | {item['published']}</span>
                    </div>
                    """, unsafe_allow_html=True)
            
            st.markdown('<br><div class="section-header">🤔 Where the Model Gets Confused</div>', unsafe_allow_html=True)
            st.markdown('<div class="help-text">Headlines with the lowest maximum probability across all classes. These are often sarcastic, double-sided, or contain highly polarizing words mixed together (e.g. "profits surge despite massive lawsuit").</div>', unsafe_allow_html=True)
            
            if audit_results["confusing_headlines"]:
                confuse_df = pd.DataFrame(audit_results["confusing_headlines"])
                confuse_df = confuse_df.rename(columns={
                    "ticker": "Ticker", "headline": "Headline", "sentiment": "Predicted Label",
                    "pos_prob": "P(Pos)", "neg_prob": "P(Neg)", "neu_prob": "P(Neu)", "max_prob": "Confidence"
                })
                
                st.dataframe(
                    confuse_df[["Ticker", "Headline", "Predicted Label", "Confidence", "P(Pos)", "P(Neg)", "P(Neu)"]],
            ...
            st.markdown('<br><div class="section-header">🔍 Live News Explorer</div>', unsafe_allow_html=True)
            st.markdown('<div class="help-text">Audit the complete live corpus. Use the column headers to sort by P(Pos), P(Neg) or P(Neu).</div>', unsafe_allow_html=True)
            
            full_data = audit_results["full_data"].copy()
            full_data = full_data.rename(columns={
                "published": "Published", "ticker": "Ticker", "company": "Company", "headline": "Headline",
                "sentiment": "Sentiment", "net_score": "Net Score",
                "pos_prob": "P(Pos)", "neg_prob": "P(Neg)", "neu_prob": "P(Neu)"
            })
            
            st.dataframe(
                full_data[["Published", "Ticker", "Company", "Headline", "Sentiment", "Net Score", "P(Pos)", "P(Neg)", "P(Neu)"]],
                use_container_width=True, height=500,
                column_config={
                    "Published": st.column_config.DatetimeColumn(format="YYYY-MM-DD HH:mm:ss", width="medium"),
                    "Headline": st.column_config.TextColumn(width="large"),
                    "Net Score": st.column_config.NumberColumn(format="%+.4f"),
                    "P(Pos)": st.column_config.NumberColumn(format="%.4f"),
                    "P(Neg)": st.column_config.NumberColumn(format="%.4f"),
                    "P(Neu)": st.column_config.NumberColumn(format="%.4f"),
                }
            )
            
    else:
        st.warning("No live news data available to audit. Check your internet connection or RSS feeds.")
